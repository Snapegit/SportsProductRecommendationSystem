...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"12px 6px","margin":"0px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","borderRadius":"12px","flexWrap":"wrap","background":"none","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"0"}'>
      <el-form-item :style='{"margin":"0 10px 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>活动编号：</div>
        <el-input v-model="formSearch.huodongbianhao" placeholder="活动编号" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>活动名称：</div>
	  ...
	  <el-button v-if="btnAuth('choujianghuodong','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 15px","margin":"0px 0px 0 0","color":"#fff","borderRadius":"4px","background":"#000","display":"inline-block","width":"auto","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/choujianghuodongAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		  ...
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    ...
		    <div :style='{"padding":"0 0px","overflow":"hidden","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto"}' class="item-info">
		      <div :style='{"width":"100%","padding":"0 0px 0 0","flexWrap":"wrap","display":"flex","order":"2"}'>
				<div class="title">{{item.huodongbianhao}}</div>
		      ...
		      <div class="desc" v-html="item.jiangxiangshezhi"></div>
		    </div>
		  </div>
		</div>
	</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.huodongbianhao != '') searchWhere.huodongbianhao = '%' + this.formSearch.huodongbianhao + '%';
        if (this.formSearch.huodongmingcheng != '') searchWhere.huodongmingcheng = '%' + this.formSearch.huodongmingcheng + '%';
		params['huodongzhuangtai'] = '启用';
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`choujianghuodong/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/choujianghuodongDetail', query: params});
      },
