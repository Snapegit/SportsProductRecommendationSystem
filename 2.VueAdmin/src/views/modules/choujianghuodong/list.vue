<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"1px solid #bababa","padding":"20px","margin":"0px","overflow":"hidden","borderRadius":"8px","background":"#fff","fontSize":"inherit"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"margin":"20px 0 5px","color":"#333","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('choujianghuodong','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('choujianghuodong','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"1px solid #bababa","width":"100%","padding":"0","margin":"30px 0 0","borderRadius":"8px","background":"rgba(255,255,255,.9)"}'>
				<el-table class="tables"
					...
					:border='true'
					v-if="isAuth('choujianghuodong','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="huodongkaishishijian"
						label="活动开始时间">
						<template slot-scope="scope">
							{{scope.row.huodongkaishishijian}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="huodongjieshushijian"
						label="活动结束时间">
						<template slot-scope="scope">
							{{scope.row.huodongjieshushijian}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('choujianghuodong','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('choujianghuodong','抽奖')" @click="choujiangjiluCrossAddOrUpdateHandler(scope.row,'cross','','','[1]','您的抽奖机会已用完！')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								抽奖
							</el-button>
							<el-button class="edit" v-if=" isAuth('choujianghuodong','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('choujianghuodong','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<choujiangjilu-cross-add-or-update v-if="choujiangjiluCrossAddOrUpdateFlag" :parent="this" ref="choujiangjiluCrossaddOrUpdate"></choujiangjilu-cross-add-or-update>
...
import choujiangjiluCrossAddOrUpdate from "../choujiangjilu/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				choujiangjiluCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			choujiangjiluCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.choujiangjiluCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','choujianghuodong');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.choujiangjiluCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.choujiangjiluCrossaddOrUpdate.init(row.id,type);
				});
			},
	...
	choujianghuodongstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'choujianghuodong/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
